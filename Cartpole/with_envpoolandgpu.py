...
import numpy as np
import random
from pre_req import *  # Ensure this includes necessary imports like neural network architecture
import envpool
...
# Initial setup
policy_net = ...  # Your DQN model
target_net = ...  # Copy of DQN model for stable targets
...
# Using EnvPool
env = envpool.make("CartPole-v1", env_type="gym", num_envs=16)
env.seed(42)
...
for epoch in range(epochs):
    ...
    while not np.all(dones):
        actions = np.array([choose_action(state, policy_net, get_epsilon(epoch)) for state in states])
        next_states, rewards, dones, truncs, infos = env.step(actions)

        for i in range(env.num_envs):
            if not dones[i] or not truncs[i]:
                td_error = rewards[i] + gamma * torch.max(policy_net(torch.tensor(next_states[i], device=device).float().unsqueeze(0))) * (1 - dones[i])
                memory.add(td_error.item(), (states[i], actions[i], rewards[i], next_states[i], dones[i] or truncs[i]))

        states = next_states
        total_rewards += rewards

        optimize_model(memory, policy_net, target_net, optimizer, batch_size, gamma, get_epsilon(epoch))

    if epoch % sync_freq == 0:
        target_net.load_state_dict(policy_net.state_dict())

    print(f"Epoch {epoch}, Total reward: {np.mean(total_rewards)}, Epsilon: {get_epsilon(epoch)}")

env.close()

# Evaluation with the trained Double DQN + PER model
num_episodes = 5
env_test = envpool.make("CartPole-v1", env_type="gym", num_envs=1)
for i in range(num_episodes):
    state = env_test.reset()
    done = False
    total_reward = 0
    while not done:
        action = choose_action(state[0], policy_net, epsilon=0)
