...
def optimize_model(memory, policy_net, target_net, optimizer, batch_size=64, gamma=0.99, beta=0.4,n_steps=10):
    ...
    n_step_returns = []
    for i in range(batch_size - n_steps + 1):  # Ensure loop doesn't exceed tensor bounds
        n_step_reward = 0
        for j in range(n_steps):
            n_step_reward += gamma**j * rewards[i + j] * (1 - dones[i + j])
        n_step_returns.append(n_step_reward)
    ...
    expected_state_action_values[:len(n_step_returns)] = n_step_returns
